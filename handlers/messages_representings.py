...
def get_start_message():
    message = "Привет! Я бот, с помощь которого можно устраивать карточные дуэли, вычислять _импосторов_ и многое дрругое!\n\n" \
              "Нужна помощь с тем, как меня использовать -> /help"
    return message
...
def get_help_message():
    ...
    message += "\n\nЧтобы получить доступ к другим командам можешь добавить меня в чат к своим друзьям и пользоваться" \
               "командами уже внутри группы! Например:\n/pidor - Чтобы узнать главного _импостора_ чата"
    return message
